...
async function loadProductsCart() {
  ...
  //Combertimos el resultado del localStorage en un array
            ...
            <div class="cart-product">
              ...
              <div class="cart-product-info">
                ...
                <p class="change-quantity">
                  ...
                  <button onclick="addProductCart(${id})">+</button>
                </p>
                <p class="cart-product-delete">
                  <button onclick="deleteProductCart(${id})">Eliminar</button>
                </p>
              </div>
            </div>
          `;
...
function deleteProductCart(idProduct) {
  ...
  localStorage.setItem(CART_PRODUCTOS, resultIdDelete);

  if (!localStorage.getItem(CART_PRODUCTOS)) {
...
function decreaseQuantity(idProduct) {
  const localStorageItems = localStorage.getItem(CART_PRODUCTOS);
  let arrayIds = localStorageItems.split(",");

  const index = arrayIds.indexOf("" + idProduct);

  arrayIds.splice(index, 1);
  localStorage.setItem(CART_PRODUCTOS, arrayIds);
  loadProductsCart();
}